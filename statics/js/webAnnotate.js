...
class LabelImage {
	...
	Initial = () => {
		...
		_nodes.toolTagsManager.addEventListener('click', this.test);
	};
	// test = () =>{
	// 	console.log("test");
	// }
	...
	SetImage = (src, memory=false) => {
		...
		_nodes.image.addEventListener('load', () => {
			...
			document.getElementById("lenlabellis").innerHTML = labels.length;

			// console.log(labels);
			// 获取包含复选框的父元素
			var parent = document.getElementById("labellis");
			...
			for (var i = 0; i < labels.length; i++) {
			    ...
			    checkbox.id = labels[i]['labelName'];
			    checkbox.name = labels[i]['labelName'];
			    checkbox.value = 1;

			    // 创建标签元素
			    var label = document.createElement("label");
			    label.appendChild(checkbox);
			    label.appendChild(document.createTextNode(labels[i]['labelName']));
	...
	CrossHairSwitch = () => {
		...
		if (_nodes.crossLine.className.indexOf('focus') > -1) {
			console.log(_nodes.crossLine.childNodes[1])
	...
	IsScreenFull = () => {
		if (this.isFullScreen) {
			// 取消全屏显示事件
			let el = document;
			let cfs = el.cancelFullScreen || el.webkitCancelFullScreen || el.mozCancelFullScreen || el.msCancelFullScreen;
